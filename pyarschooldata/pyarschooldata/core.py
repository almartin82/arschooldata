"""
Core functions wrapping arschooldata R package via rpy2.
"""
...
def _get_pkg():
    ...
    global _pkg
    if _pkg is None:
        _pkg = importr("arschooldata")
    return _pkg


def fetch_enr(end_year: int) -> pd.DataFrame:
    """
    Fetch Arkansas school enrollment data for a single year.
...
def fetch_enr_multi(end_years: list[int]) -> pd.DataFrame:
    """
    Fetch Arkansas school enrollment data for multiple years.
    ...
    --------
    >>> import pyarschooldata as ar
    >>> df = ar.fetch_enr_multi([2020, 2021, 2022])
    """
    pkg = _get_pkg()
    with localconverter(robjects.default_converter + pandas2ri.converter):
...
def get_available_years() -> dict:
    ...
    with localconverter(robjects.default_converter + pandas2ri.converter):
        ...
        # Fallback: check for names attribute
        if hasattr(r_result, "names") and callable(getattr(r_result.names, "__iter__", None)):
