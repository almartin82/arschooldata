# ==============================================================================
# LIVE Pipeline Tests for arschooldata
# ==============================================================================
#
...
test_that("Arkansas DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Arkansas DOE URL and verify HTTP 200")
})
...
test_that("Can download Arkansas enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Arkansas enrollment file", {
  skip_if_offline()
...
test_that("arschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- arschooldata::get_available_years()
  ...
  tryCatch({
    raw <- arschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- arschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- arschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- arschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- arschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- arschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- arschooldata:::get_cache_path(2024, "enrollment")
